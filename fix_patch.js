...
const reviewsByMonth = MONTHS_SHORT.map(name => {
    if (name === 'Apr') {
        return {
            mapValue: {
                fields: {
    ...
    return {
        mapValue: {
            fields: {
                name: { stringValue: name },
                Import: { integerValue: "0" },
                Export: { integerValue: "0" }
            }
        }
    };
});
...
const payload = {
    fields: {
        reviewsByMonth: { arrayValue: { values: reviewsByMonth } },
...
const options = {
    ...
    path: `/v1/projects/logimaster-cfmoto/databases/(default)/documents/data_stage_reports/9b04de33-6b65-419f-b31b-085651116c94?updateMask.fieldPaths=reviewsByMonth&updateMask.fieldPaths=monthlyDuties`,
    method: 'PATCH',
    headers: {
