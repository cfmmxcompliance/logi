...
async function run() {
    ...
    console.log("🚀 Extracting FINAL 13 edge cases...");
    ...
    for (const dipc of dipcFolders) {
        ...
        for (const mc of mcFolders) {
            ...
            if (zips.length > 0) {
                ...
                if(expZips.length === 0) {
                     console.log(`  ❌ NO EXPENSE ZIP FOUND in ${mc.name}`);
                     continue;
                }
                ...
                const tempZipPath = path.join(os.tmpdir(), `temp_edge_${Math.random().toString(36).substring(7)}.zip`);
                ...
                for (const entry of entries) {
                    ...
                    if (actualName.includes('_PED') || actualName.includes(' A_PED') || actualName.includes('SIM.pdf')) {
                        ...
                        if(foundPedimentoName.includes('SIM')) foundPedimentoName = mc.name + " " + foundPedimentoName;
                    }
                ...
                if (foundPedimentoName) {
                    ...
                    console.log(`  ❌ Could not find an internal A_PED or PED_SIM file!`);
                }
            }
        }
    }
