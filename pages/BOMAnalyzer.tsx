...
import {
  Upload, Terminal, FileSpreadsheet, CheckCircle2, AlertTriangle,
  XCircle, Download, ChevronRight, Cpu, RefreshCw, Trash2,
  AlertCircle, FileSearch, Zap, BarChart3, X
} from 'lucide-react';
...
export const BOMAnalyzer: React.FC = () => {
  ...
  // ── Drop handlers ─────────────────────────────────────────────────────
  const onDrop = useCallback((e: React.DragEvent, isMaster = false) => {
    e.preventDefault();
    if (isMaster) setMasterDragging(false); else setDragging(false);
    const file = e.dataTransfer.files[0];
    if (!file) return;
    if (isMaster) handleMasterFile(file);
    else handleBOMFile(file);
  }, [rawRows, normalizedRows, dedupedRows]); // eslint-disable-line
        ...
        <div className="w-72 flex-shrink-0 bg-slate-900 border-r border-slate-800 flex flex-col p-4 gap-3 overflow-y-auto">
          ...
          {(step === 'deduped' || step === 'crossed' || step === 'done') && (
            <div className="space-y-1 pt-2 border-t border-slate-800">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">4 · Exportar</p>
