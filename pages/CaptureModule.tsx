  ...
  const generateEnrichedPayload = () => {
      const enriched = vinPayload.map(v => {
          ...
          return {
              ...
              puAduana:         matchedBom?.puAduana || matchedBom?.valorUsdUnitario || 0,
              pesoBruto:        matchedBom?.pesoBrutoUnitarioKg || 0,
              pesoNeto:         matchedBom?.pesoNetoUnitarioKg || 0,
              pesoBrutoLb:      matchedBom?.pesoBrutoUnitarioLb || 0,
              pesoNetoLb:       matchedBom?.pesoNetoUnitarioLb || 0,
              volumen:          matchedBom?.volumenUnitario || 0,
              valAcero:         matchedBom?.ValAcero || 0,
              pesoAcero:        matchedBom?.pesoAcero || 0,
              expo:             matchedBom?.expo || '',
              unidadAduana:     matchedBom?.unidadAduana || '06 PIEZA',
              cantidadAduana:   matchedBom?.cantidadAduana ?? 1,
              clavePedimento:   matchedBom?.clavePedimento || '',
              incoterm:         matchedBom?.incoterm || 'FCA',
              claveProductoSat: matchedBom?.claveProductoSat || '25101503',
              objetoImpuesto:   matchedBom?.objetoImpuestoSat || '01- No objeto de impuesto',
              unidadMedidaSat:  matchedBom?.unidadMedidaSat || 'H87 Pieza',
              usoCfdi:          matchedBom?.usoCfdiSat || 'S01 Sin Efectos fiscales',
              mid:              matchedBom?.mid || '',
              testGroupNameNo:  matchedBom?.testGroupNameNo || '',
              bomFound:         !!matchedBom,
          };
      });
