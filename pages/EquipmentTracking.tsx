...
export const EquipmentTracking = () => {
    ...
    useEffect(() => {
        const existing = storageService.getEquipmentTracking();
        if (existing.length > 0) {
            setRecords(existing);
        } else {
            setIsLoading(true);
            storageService.loadEquipmentTracking().then(() => {
                            ...
                            {isLoading ? (
                                <tr>
                                    <td colSpan={16} className="py-20 text-center text-slate-400">
                                        ...
                                        <p className="text-sm font-medium">Cargando registros de Equipment...</p>
                                    </td>
                                </tr>
