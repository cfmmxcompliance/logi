...
export const Operations = () => {
    ...
    const confirmBulkDelete = async () => {
        if (!isAdmin) return;
        try {
            await storageService.deleteShipments(Array.from(selectedIds));
            setSelectedIds(new Set());
                ...
                <div className="flex gap-2">
                    ...
                    {selectedIds.size > 0 && isAdmin && (
                        <button
                            ...
                            className="bg-red-50 border border-red-200 text-red-700 px-4 py-2 rounded-lg flex items-center gap-2 shadow-sm hover:bg-red-100 transition-all"
                        >
                ...
                <div className="overflow-auto flex-1">
                    <table className="w-full text-xs text-left border-collapse">
                        <thead className="bg-slate-50 text-slate-600 font-bold sticky top-0 z-10 shadow-sm whitespace-nowrap">
                            <tr>
                                <th className="px-3 py-3 border-b border-r border-slate-200 bg-slate-50 sticky left-0 z-20 w-[40px] text-center">
                                    <input
                                        type="checkbox"
                                ...
                                <th className="px-3 py-3 border-b border-r border-slate-200 bg-slate-50 sticky left-[40px] z-20">Action</th>
