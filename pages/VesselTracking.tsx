...
export const VesselTracking = () => {
    ...
    const confirmBulkDelete = async () => {
        if (!isAdmin) return;
        try {
            await storageService.deleteVesselTrackings(Array.from(selectedIds));
            setSelectedIds(new Set());
                        ...
                        <tbody className="divide-y divide-slate-100 whitespace-nowrap">
                            {filteredRecords.map((r) => (
                                <tr key={r.id} className="hover:bg-slate-50 transition-colors">
                                    ...
                                    <td className="px-3 py-2 border-r border-slate-100 sticky left-[104px] bg-white hover:bg-slate-50 font-bold">{r.refNo}</td>
