...
export const XMLInvoiceExtractor: React.FC = () => {
    ...
    useEffect(() => {
        const fetchTargetedData = async () => {
            ...
            setLoading(true);
            try {
                const refreshedInvoices = await storageService.searchCFDIInvoicesByPrefix(terms);
    ...
    const processFiles = async (files: File[]) => {
        ...
        // --- DUPLICATE DETECTION ---
        // Verify duplicates directly against Firebase to avoid loading the whole database locally
        const parsedUUIDs = parsedFiles.map(f => f.uuid).filter(Boolean);
        const existingUUIDs = await storageService.checkCFDIExistsByUUID(parsedUUIDs);
